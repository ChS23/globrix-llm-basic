  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl md:text-2xl text-[var(--foreground)]">Доступные объекты</h2>
          <p className="text-xs md:text-sm text-[var(--foreground-muted)] mt-1">Найдено: {items.length}</p>
        </div>
...
function ApartmentCard({ apartment, index }: { apartment: Apartment; index: number }) {
        ...
        <div className="p-4 md:p-5">
          ...
          <div className="flex items-center justify-between mb-3 md:mb-4">
            <div className="flex items-center gap-2 md:gap-3">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-gradient-to-br from-[var(--accent-dark)]/20 to-[var(--accent-light)]/10 border border-[var(--border-accent)] flex items-center justify-center">
                <span className="text-xs md:text-sm font-bold text-[var(--accent)] uppercase">{apartment.type}</span>
              </div>
              <div>
                <span className="text-xs md:text-sm font-mono text-[var(--foreground-muted)]">{apartment.id}</span>
                {apartment.floor && (
                  <p className="text-[10px] md:text-xs text-[var(--foreground-subtle)]">Этаж {apartment.floor}</p>
                )}
              </div>
            </div>
          ...
          {/* Price - prominent */}
          <div className="mb-3 md:mb-4 pb-3 md:pb-4 border-b border-[var(--border)]">
          ...
          <div className="flex flex-wrap gap-3 md:gap-4 mb-3 md:mb-4">
            <div className="flex items-center gap-1.5 md:gap-2">
              <AreaIcon />
              <span className="text-xs md:text-sm text-[var(--foreground)]">{apartment.area} м²</span>
            </div>
            {apartment.bedrooms !== undefined && (
              <div className="flex items-center gap-1.5 md:gap-2">
                <BedroomIcon />
                <span className="text-xs md:text-sm text-[var(--foreground)]">{apartment.bedrooms} спальн.</span>
              </div>
            )}
            {apartment.bathrooms !== undefined && (
              <div className="flex items-center gap-1.5 md:gap-2">
                <BathroomIcon />
                <span className="text-xs md:text-sm text-[var(--foreground)]">{apartment.bathrooms} ванн.</span>
              </div>
            )}
          </div>
          ...
          {(apartment.developer || apartment.completion_date) && (
            <div className="flex flex-wrap gap-3 md:gap-4 text-xs md:text-sm text-[var(--foreground-muted)] mb-3 md:mb-4">
              {apartment.developer && (
  ...
  return (
    ...
    >
      {/* Image */}
      <div className="relative h-40 md:h-48 bg-[var(--background-elevated)]">
        <img
        ...
        <div className="absolute top-2 md:top-3 left-2 md:left-3">
          <span className="px-2 md:px-3 py-1 md:py-1.5 rounded-lg bg-[var(--background)]/80 backdrop-blur-sm text-[10px] md:text-xs font-semibold text-[var(--foreground)] uppercase tracking-wide">
            {apartment.type}
          </span>
        </div>
      </div>
      ...
      <div className="p-4 md:p-5">
        ...
        <div className="flex items-center justify-between mb-2 md:mb-3">
          <span className="text-xs md:text-sm font-mono text-[var(--accent)]">{apartment.id}</span>
          {apartment.floor && (
            <span className="text-[10px] md:text-xs text-[var(--foreground-subtle)]">Этаж {apartment.floor}</span>
          )}
        </div>
        ...
        {(apartment.developer || apartment.completion_date) && (
          <div className="pt-3 md:pt-4 border-t border-[var(--border)] space-y-1.5 md:space-y-2">
            {apartment.developer && (
              <div className="flex items-center gap-2 text-xs md:text-sm">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-[var(--foreground-subtle)] md:w-3.5 md:h-3.5">
                  <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/>
                  <line x1="9" y1="3" x2="9" y2="21"/>
                </svg>
        ...
        {/* Action Button */}
        <button className="btn-primary w-full mt-4 md:mt-5 flex items-center justify-center gap-2">
          <span>Подробнее</span>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
            <line x1="5" y1="12" x2="19" y2="12"/>
            <polyline points="12,5 19,12 12,19"/>
...
function DetailItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-1.5 md:gap-2 p-1.5 md:p-2 rounded-lg bg-[var(--background-elevated)]">
      <div className="text-[var(--accent)]">{icon}</div>
      <div className="flex-1 min-w-0">
        <div className="text-[9px] md:text-[10px] text-[var(--foreground-subtle)] uppercase tracking-wider">{label}</div>
        <div className="text-xs md:text-sm text-[var(--foreground)] font-medium truncate">{value}</div>
      </div>
    </div>
  );
}
